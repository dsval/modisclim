...
#' @param  x, Raster* object with z time dimension
...
aggRaster<-function(x,func='mean',xout='monthy',inmem=FALSE,cl=NULL,outdir=getwd(),varnam='sm', ... ){
	###########################################################################
	...
	# }
			...
			if(xout=='monthy'){
				...
				#to aggregate data from subdaily to daily
				y<-as.numeric(unique(format(getZ(x),'%Y')))
				ind.months<-seq(as.Date(getZ(x)[1]),as.Date(getZ(x)[length(getZ(x))]), by="day")					
				nm <-length(ind.months)
				# mold
			...
			#pb <- pbCreate(bs$n)
			#if(bs$n==1){
			#	pb <- txtProgressBar(min=1,max = 2, style = 1)
			#}else{
